...
bool MapDriver(HANDLE IntelDrvHandle, BYTE* RawImage)
{
	...
	uint64_t GhostDriverBase = utils::GetKernelModuleAddress("dump_diskdump.sys");
	if (!GhostDriverBase)
	{
		Log(L"[-] failed to resolve base of driver...\n");
		return false;
	}
	if (!intel_driver::ReadMemory(IntelDrvHandle, GhostDriverBase, OriginalMemory, ImageSize))
	{
		Log(L"[-] failed to read driver pte" << std::endl);
		return false;
	}
	...
	{
		Log(L"[*] cant map over the specefied driver , image size is too small" << std::endl);
		return false;
	}
	...
	for (uint64_t CurrentAddress = GhostDriverBase; CurrentAddress < GhostDriverEnd; CurrentAddress += USN_PAGE_SIZE)
	{
		...
		{
			Log(L"[-] failed to patch driver pte" << std::endl);
			return false;
		}
	}
	Log(L"[*] marked ghost driver pages as rwx" << std::endl);

	// our target driver is ready , write over the driver memory ;
